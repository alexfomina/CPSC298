...
/* ?? add initializers for member variables 
...
// bShowboat is false. 
// 
// When bShowboat is false, the grid is rendered as follows. Note that location 2, 0 contains a '~'/ character.
// 
// |*|~|~|~|~|
// |~|~|~|~|~|
// |~|*|*|~|~|
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/ //
    for (int iRow = 0; iRow < this->m_nGridRows; ++iRow)
    {
        ...
        for (int iColumn = 0; iColumn< this->m_nGridColumns; ++iColumn)
        {
            if ('X' == this->a_c_Grid[iRow][iColumn] || 'V' == this->a_c_Grid[iRow][iColumn])
            {   
            ...
            Boat location secret, '~' */
                if (bShowBoat)
                {
                std::cout << this->a_c_Grid[iRow][iColumn] << "|";
                }
                else
                {
                std::cout << "~" << "|";
                }
            } else if('*' == this->a_c_Grid[iRow][iColumn]) 
            {
                std::cout << "*" << "|";
            }
            else
...
void Game::placeBoatRandomly()
{
    ...
    a_c_Grid[m_iRowBoat][m_iColumnBoat] = 'V';
    #if defined(DIAG)
    std::cout << "DIAG: Boat location (Row/Column): " << this->m_iRowBoat << "; "<< this->m_iColumnBoat << std::endl;
    #endif 
}
// Implementation notes on Game::placeBoat
// Game::placeBoat is used for testing purposes only. During gameplay,  Game::placeBoatRandomly() is used instead.
void Game::placeBoat()
{
    int iRow = 0;
    int iColumn = 0;
    ...
    a_c_Grid[iRow][iColumn] = 'V';
    /* ?? Assign value to this->m_iRowBoat ?? */
    iRow = this->m_iRowBoat;
    /* ?? Assign value to this->m_iRowColumn ?? */
    iColumn = this->m_iColumnBoat;
    return;
}
bool Game::isOnTarget(int iRow, int iColumn)
{
    ...
    // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
    if ((iRow == this->m_iRowBoat) && (iColumn == m_iColumnBoat)) 
    {
        bHit = true;
    }
    return bHit;
...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
    ...
    // the target boat. 
    // /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
    if ( (iRow - this->m_iRowBoat <= 1) && (iColumn - this->m_iColumnBoat <= 1) )
    {
        bClose = true;
    }
    return bClose;
...
/* ?? Assign 'X' to a_cGrid at location (row/column) of Battle Boat ?? */
    this->a_c_Grid[this->m_iRowBoat][this->m_iColumnBoat]= 'X';
    return;
}
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
    ...
    std::cin >> iRow;
    while(iRow >= k_nGridRowsDefault) {
        std::cout << "Please re-enter grid row number: " << std::endl;
        std::cin >> iRow;
    }
/* ?? Use std::cin ?? */
    ...
    std::cin >>iColumn;
    while(iColumn >= k_nGridColumnsDefault) {
        std::cout << "Please re-enter grid column number: " << std::endl;
        std::cin >> iColumn;
    }
/* ?? Use std::cin ?? */
